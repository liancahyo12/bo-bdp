$(document).ready(function(){
    $('#jenis_pengajuan').on('change', function() {
        ...
        if ( this.value <= 3){
            ...
            document.getElementById("form-pengajuan").innerHTML = "<div><label for='jumpc'>Jumlah Petty Cash(dalam rupiah)*</label><input class='form-control angka' name='jumpc' type='number' id='jumpc' required></input></div>       <br><h3>Kebutuhan</h3><table class='table' id='dynamic'><tr id='row'><td><label for='jenistr"+no+"'>Jenis Transaksi*</label><input class='form-control' type='text' name='jenistr[]' id='jenistr' required></td><td><br><button type='button' id='tambah' class='btn btn-success tambah4'>Tambah</button></td></tr></table> <label for='catatan'>Catatan*</label><input class='form-control' type='text' name='catatan' id='catatan' required><h3>Tujuan</h3><label for='namarek'>Nama Rekening*</label><input class='form-control' name='namarek' type='text' id='namarek' required><label for='norek'>No Rekening*</label>            <input class='form-control' name='norek' type='number' id='norek' required><label for='bank'>Bank*</label>           <input class='form-control' name='bank' type='text' id='Bank' required>"
            $(document).on('click', '.tambah4', function(){
                no++;
                $('#dynamic').append("<tr id='row"+no+"'><td><input class='form-control' type='text' name='jenistr[]' id='jenistr"+no+"' required></td><td><button type='button' id='tambah"+no+"' class='btn btn-success tambah4'>+</button><button type='button' id='"+no+"' class='btn btn-danger btn_remove'>x</button></td></tr>");
            });
            ...
            document.getElementById("form-pengajuan").innerHTML = "<label for='perusahaan'>Perusahaan*</label><input class='form-control' name='perusahaan' type='text' id='perusahaan' required><label for='alamat'>Alamat*</label><input class='form-control' name='alamat' type='text' id='alamat' required><label for='notelepon'>No Telepon*</label><input class='form-control' name='notelepon' type='number' id='notelepon' required><label for='kontak'>Nama Kontak*</label><input class='form-control' name='kontak' type='text' id='kontak' required><label for='email'>Email*</label><input class='form-control' name='email' type='text' id='email' required>        <table class='table' id='dynamic'><tr id='row'><td><label for='pembelian'>Pembelian*</label><input class='form-control' type='text' name='pembelian[]' id='pembelian' required></td><td><label for='nominal'>Nominal*</label><input class='form-control angka' type='number' name='nominal[]' id='nominal' required></td><td><br><button type='button' id='tambah' class='btn btn-success tambah5'>Tambah</button></td></tr></table>           <table class='table'><tr><td style='width:41%'><label for='total'>Total</label> </td><td> <input class='form-control' type='text' name='total' id='total' readonly></td><td style='width:18%'> </td></tr></table>      <table class='table'> <tr><td><label for='dpp'>DPP*</label></td><td><input class='form-control angka' type='number' name='dpp' id='dpp' required></td></tr><tr><td><label for='ppn'>PPN*</label></td><td><input class='form-control angka' type='number' name='ppn' id='ppn' required> </td></tr></table>        <label for='catatan'>Catatan*</label><input class='form-control' type='text' name='catatan' id='catatan' required><br>"
            $(document).on('click', '.tambah5', function(){
                no++;
                $('#dynamic').append("<tr id='row"+no+"'><td><input class='form-control' type='text' name='pembelian[]' id='pembelian"+no+"' required></td><td><input class='form-control angka' type='number' name='nominal[]' id='nominal"+no+"' required></td><td><button type='button' id='tambah"+no+"' class='btn btn-success tambah5'>+</button><button type='button' id='"+no+"' class='btn btn-danger btn_remove'>x</button></td></tr>");
            });
