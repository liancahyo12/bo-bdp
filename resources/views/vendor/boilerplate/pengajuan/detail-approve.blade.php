...
@section('content')
    <x-boilerplate::form :route="['boilerplate.update-approve-pengajuan', $pengajuan->ida]" method="put" files>
    @csrf
        ...
        <x-boilerplate::card>
            ...
            @foreach ( $approvepengajuan as $approvepengajuan)
                <div class="direct-chat-msg">
                    <div class="direct-chat-infos clearfix">
                        ...
                        <span class="badge badge-pill badge-secondary float-left">{{ $approvepengajuan->kode }}</span>
                        @if ($approvepengajuan->statuss == 2)
                            <span class="badge badge-pill badge-success float-right">disetujui</span>
                        @endif
                        @if ($approvepengajuan->statuss == 3)
                            <span class="badge badge-pill badge-warning float-right">revisi</span>
                        @endif
                        @if ($approvepengajuan->statuss == 4)
                            <span class="badge badge-pill badge-danger float-right">ditolak</span>
                        @endif
                        <span class="direct-chat-timestamp float-right">{{ $approvepengajuan->waktu_komentar }}&nbsp;</span>
                    </div>
                    <!-- /.direct-chat-infos -->
                    <img src="{{ App\Models\Boilerplate\User::where('id', $komentar->uid)->first()->avatar_url }}" class="direct-chat-img" alt="{{ $approvepengajuan->first_name }}" width="30" height="30">
                    <!-- /.direct-chat-img -->
            ...
            <div @if ($pengajuan->approve_status==2 || $pengajuan->approve_status==3 || $pengajuan->approve_status==4)
                style='display:none;'
                @endif>
            ...
            <div>
                <input class="btn btn-warning" name="submitbutton" type="submit" value="Revisi" @if ($pengajuan->approve_status==2 || $pengajuan->approve_status==3 || $pengajuan->approve_status==4)
                style='display:none;'
                @endif>
                &nbsp;
                <input class="btn btn-danger" name="submitbutton" type="submit" value="Tolak" @if ($pengajuan->approve_status==2 || $pengajuan->approve_status==3 || $pengajuan->approve_status==4)
                style='display:none;'
                @endif>
                &nbsp;
                <input class="btn btn-primary" name="submitbutton" type="submit" value="Setujui" @if ($pengajuan->approve_status==2 || $pengajuan->approve_status==3 || $pengajuan->approve_status==4)
                style='display:none;'
                @endif>
            </div>
