...
class RequestsuratkeluarsayaDatatable extends Datatable
{
    public $slug = 'requestsuratkeluarsaya';

    public function datasource()
    {
        return Request_surat_keluar::leftJoin('jenis_surats', 'request_surat_keluars.jenis_surat_id', 'jenis_surats.id')->leftJoin('users', 'users.id', 'request_surat_keluars.user_id')->where([['user_id','=', Auth::user()->id], ['status', '=', '1'], ['send_status', '=', '1']])->orderByDesc('request_surat_keluars.updated_at')->get(['request_surat_keluars.id',
        'perihal',
        'request_status',
        'request_time',
        'users.first_name',
        ...
        return [
            ...
            Column::add()
                ->actions(function(Request_surat_keluar $request_surat_keluar) {
                        return join([
                        Button::show('boilerplate.surat-keluar-request-saya.edit', $request_surat_keluar->id),           
                    ]); 
                }),
        ];
    }
